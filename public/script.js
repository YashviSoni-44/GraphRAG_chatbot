...
// // ========================
// // File Selection & Chat History Rendering
// // ========================

// function setActiveFile(filename) {
//   if (!filename || filename === activeFile) return;  // Avoid redundant reloads
//   activeFile = filename;
//   activeFileSpan.textContent = filename.split('/').pop();  // show only basename
...
//     const speaker = entry.role === "user" ? "You" : "Bot";
//     chatbox.innerHTML += `<p><b>${speaker}:</b> ${entry.content}</p>`;
//   });
...
// function doUpload(formData, files) {
...
//         const newlyAdded = [];
//         for (const file of files) {
//           // Use full relative path if available, else fallback to file name
//           const nameOrPath = file.webkitRelativePath || file.name;
//           if (!uploadedFiles.includes(nameOrPath)) {
//             uploadedFiles.push(nameOrPath);
//             addFileToSidebar(nameOrPath);
//             newlyAdded.push(nameOrPath);
//           }
//         }
//         // Auto-select first new file if none active or active no longer valid
//         if ((!activeFile || !uploadedFiles.includes(activeFile)) && newlyAdded.length) {
//           setActiveFile(newlyAdded[0]);
//         }
...
//   li.textContent = fileName.split('/').pop(); // Show basename only
//   li.dataset.fullname = fileName;             // Store full path in dataset
...
//   }
//   if (!contextText.trim()) {
//     chatbox.innerHTML += `<p><b>Bot:</b> ❌ No relevant context found for this file.</p>`;
//     chatbox.scrollTop = chatbox.scrollHeight;
//     return;
//   }
...
//     } else {
//       chatbox.innerHTML += `<p><b>Bot:</b> Error: ${data.error || "Unknown error"}</p>`;
//       chatbox.scrollTop = chatbox.scrollHeight;
//     }
//   } catch (err) {
...
// // ========================
// // Persist uploadedFiles in localStorage
// // ========================
...
//   localStorage.setItem('activeFile', filename);  // persist last active file
...
//           }
//         }
//         saveUploadedFiles();  // persist updated file list
...
//   // ===== Restore entire uploaded files list from localStorage on page load =====
...
//   // ===== Restore last active file on page load =====
...
//     // fallback: select first file if activeFile not set or not found
//     setActiveFile(uploadedFiles[0]);
//   }
// });

// // ========================
// // Expose key functions globally to fix `uploadFiles is not defined` error
// // ========================
...
// // ========================
// // Chat Messaging with typing effect
// // ========================

// const TYPING_DELAY_MS = 600; // time user typing indication is visible (simulate)

// // Show "You are typing..." message temporarily
...
//   // Animate the "You are typing..." for a small delay to mimic UX
//   setTimeout(async () => {
//     hideUserTypingIndicator();

//     // Render user message bubble from right
...
//     // Bot typing indicator bubble (left side)
...
// document.addEventListener('DOMContentLoaded', () => {
//   // Attach event listeners for uploads and chat form
//   const uploadFilesBtn = document.getElementById('uploadFilesBtn');
//   const uploadFolderBtn = document.getElementById('uploadFolderBtn');
//   if (uploadFilesBtn) uploadFilesBtn.addEventListener('click', uploadFiles);
//   if (uploadFolderBtn) uploadFolderBtn.addEventListener('click', uploadFolder);
...
//   // Restore uploaded files sidebar
//   const savedFiles = loadUploadedFiles();
//   for (const fileName of savedFiles) {
//     if (!uploadedFiles.includes(fileName)) {
//       uploadedFiles.push(fileName);
...
//     }
//   }
//   // Restore active chat file & chat history
//   const savedActiveFile = localStorage.getItem('activeFile');
//   if (savedActiveFile && uploadedFiles.includes(savedActiveFile)) {
//     setActiveFile(savedActiveFile);
//   } else if (uploadedFiles.length > 0) {
...
//   messageDiv.classList.add('chat-message');
//   messageDiv.classList.add(role === 'user' ? 'user' : 'bot');
//   messageDiv.innerHTML = `<b>${role === 'user' ? 'You' : 'Bot'}:</b> ${escapeHTML(content)}`;
//   return messageDiv;
// }
